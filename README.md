...
## 本分支修改说明：
本分支基于[原版语燕输入法](https://github.com/gurecn/YuyanIme)进行以下优化：

### 词库导入导出
+ 新增**单独的词库导入/导出功能**（设置 → 导出词库 / 导入词库）
+ 导出：将用户词典（`*.userdb`）打包为 zip 文件
+ 导入：自动识别 zip 内的词库文件，**自动重命名为 `pinyin.userdb`**，兼容小狼毫、雾凇拼音、白霜拼音等 PC 端词库
+ 导入后自动刷新 Rime 引擎，无需重启应用

### 剪贴板优化
+ 剪贴板上限改为 **99999**
+ 点击剪贴板条目时**自动置顶**（更新时间戳排到第一位）

### 输入卡顿优化
+ Rime JNI 调用改为**异步处理**，不再阻塞 UI 线程
+ 添加 **100ms 防抖**，快速连按只处理最后一个按键
+ 候选词 page_size 从 100 减为 20，降低大词库查询开销

## Windows 词库导入手机端教程

### 前提条件
+ PC 端使用小狼毫（weasel）或其他基于 Rime 的输入法
+ 手机端使用本版本语燕输入法

### 操作步骤
1. **PC 端找到词库文件夹**
   打开 `C:\Users\你的用户名\AppData\Roaming\Rime\`
   找到你的词库文件夹，例如：
   - 白霜拼音：`rime_frost.userdb`
   - 雾凇拼音：`rime_ice.userdb`
   - 原版拼音：`pinyin.userdb`

2. **压缩为 zip**
   右键点击该文件夹 → **发送到 → 压缩文件夹**
   （注意：是右键整个文件夹压缩，不是进文件夹里全选压缩）

3. **传到手机**
   通过微信、QQ、数据线等方式把 zip 文件传到手机

4. **导入词库**
   打开语燕输入法设置 → 点击「导入词库」→ 选择传过来的 zip 文件
   应用会自动将词库重命名为 `pinyin.userdb` 并生效

### 手机端词库导出到 PC
1. 语燕输入法设置 → 点击「导出词库」→ 保存 zip 文件
2. 把 zip 传到 PC，解压得到 `pinyin.userdb` 文件夹
3. 复制到 `C:\Users\你的用户名\AppData\Roaming\Rime\`
4. 如果 PC 端用的不是原版拼音方案，需要重命名（如改为 `rime_frost.userdb`）
5. PC 端执行 `WeaselDeployer.exe /deploy` 重新部署
